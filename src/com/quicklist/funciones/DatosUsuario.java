    ...
    public DatosUsuario(String usuario, String tipo, Statement declaracion, JLabel jLabel1, JLabel jLabel2, JLabel jLabel3) {
        ...
        try {
            ...
            if ("Aprendiz".equals(tipo)) {
                resultado = declaracion.executeQuery("select Nombre,Primer_Apellido,Documento_De_Identidad  from T_Informacion_Aprendices where Documento_De_Identidad=" + usuario + ";");
            }
            if ("Instructor".equals(tipo)) {
                resultado = declaracion.executeQuery("select Nombre,Primer_Apellido,Documento_De_Identidad  from T_Informacion_Funcionarios where Documento_De_Identidad=" + usuario + " and cargo='INSTRUCTOR';");
            }
            if ("Administrador".equals(tipo)) {
                resultado = declaracion.executeQuery("select Nombre,Primer_Apellido,Documento_De_Identidad  from T_Informacion_Funcionarios where Documento_De_Identidad=" + usuario + " and cargo='ADMINISTRADOR';");
            }
